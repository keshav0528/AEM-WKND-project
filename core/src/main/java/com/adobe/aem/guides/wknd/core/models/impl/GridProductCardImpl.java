...
public class GridProductCardImpl extends CustomComponentImpl implements GridProductCard {

    protected static final String RESOURCE_TYPE = "wknd/components/grid_product";
    ...
    public void init() {
        ...
        if (null != res && res.hasChildren()) {
            ...
            while (cards.hasNext()) {
                ...
                details.setButton(card.getValueMap().get("button", String.class));
                details.setPrice(card.getValueMap().get("price", String.class));
                details.setTitle(card.getValueMap().get("title", String.class));
                details.setImage(card.getValueMap().get("image", String.class));
                details.setCategory(card.getValueMap().get("category", String.class));
                details.setProductType(card.getValueMap().get("producttype", String.class));
                details.setGender(card.getValueMap().get("gender", String.class));
                details.setSize(card.getValueMap().get("size", String.class));
                details.setColor(card.getValueMap().get("color", String.class));
